...
P_grid_BESS = cp.Variable(n_steps)
...
# 3. BESS power limits
for t in range(n_steps):
    constraints.append(P_PV_BESS[t] + P_grid_BESS[t] <= bess_power_limit)  # Charging limit
    constraints.append(P_BESS_consumer[t] + P_BESS_grid[t] <= bess_power_limit)  # Discharging limit
...
for t in range(n_steps):
    ...
    constraints.append(P_BESS_grid[t] >= 0)
