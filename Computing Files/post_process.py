...
def compute_revenues(results, data):
    ...
    # Handle EV related flows, default to zeros if not present
    ...
    pv_to_ev_rev = P_PV_ev * (data['pi_ev'] - data['lcoe_pv']) * data['delta_t']
    bess_to_ev_rev = P_BESS_ev * (data['pi_ev'] - data['lcoe_bess']) * data['delta_t']
    grid_to_ev_rev = P_grid_ev * data['pi_ev'] * data['delta_t']
    ...
    grid_buy_cost = - (results['P_grid_consumer_vals'] + results['P_grid_BESS_vals'] + P_grid_ev) * data['grid_buy_price'] * data['delta_t']
    penalty_per_step = -1e5 * results['slack_vals'] * data['delta_t']  # Penalty adjusted for energy units (kWh)
    total_net_per_step = pv_to_consumer_rev + pv_to_ev_rev + pv_to_grid_rev + pv_to_bess_cost + bess_to_consumer_rev + bess_to_grid_rev + bess_to_ev_rev + grid_buy_cost + penalty_per_step + grid_to_ev_rev
    
    # Add EV charging revenues (real cash inflow)
    ev_charging_revenue = (P_PV_ev + P_BESS_ev + P_grid_ev) * data['pi_ev'] * data['delta_t']
    ...
    revenues = {
        ...
        'bess_to_ev_rev': bess_to_ev_rev,
        'bess_to_grid_rev': bess_to_grid_rev,
        'grid_to_ev_rev': grid_to_ev_rev,
    ...
    total_ev_demand = np.sum(data.get('ev_demand', np.zeros(data['n_steps']))) * data['delta_t']
    renewable_to_ev = np.sum(P_PV_ev + P_BESS_ev) * data['delta_t']
